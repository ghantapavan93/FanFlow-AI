...
const MATCH_LOOKUP: Record<
  string,
  { teamA: string; flagA: string; teamB: string; flagB: string; label: string; date: string; group: string }
> = {
  'match-11': { teamA: 'Netherlands', flagA: '🇳🇱', teamB: 'Japan', flagB: '🇯🇵', label: 'Match 11', date: 'Sun, 14 Jun · 3:00 PM', group: 'Group F' },
  'match-22': { teamA: 'England', flagA: '🏴󠁧󠁢󠁥󠁮󠁧󠁿', teamB: 'Croatia', flagB: '🇭🇷', label: 'Match 22', date: 'Wed, 17 Jun · 3:00 PM', group: 'Group L' },
  'match-43': { teamA: 'Argentina', flagA: '🇦🇷', teamB: 'Austria', flagB: '🇦🇹', label: 'Match 43', date: 'Mon, 22 Jun · 12:00 PM', group: 'Group J' },
  'match-57': { teamA: 'Japan', flagA: '🇯🇵', teamB: 'Sweden', flagB: '🇸🇪', label: 'Match 57', date: 'Thu, 25 Jun · 6:00 PM', group: 'Group F' },
  'match-70': { teamA: 'Jordan', flagA: '🇯🇴', teamB: 'Argentina', flagB: '🇦🇷', label: 'Match 70', date: 'Sat, 27 Jun · 9:00 PM', group: 'Group J' },
  'match-78': { teamA: 'TBD', flagA: '⚽', teamB: 'TBD', flagB: '⚽', label: 'Match 78', date: 'Tue, 30 Jun · 12:00 PM', group: 'Round of 32' },
  'match-final': { teamA: 'Champion', flagA: '🏆', teamB: 'Champion', flagB: '🏆', label: 'Match 104', date: 'Sat, 19 Jul · 7:00 PM', group: 'Final' },
}
...
export default function DiscoverDetailPage() {
  ...
  const matchId = search?.get('match') ?? 'match-final'
  const match = MATCH_LOOKUP[matchId] ?? MATCH_LOOKUP['match-final']
    ...
    <div className="min-h-screen page-bg page-enter">
      ...
      <header className="page-header">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-4">
          ...
          <Link href={`/discover/${eventId}`} className="btn-ghost !min-h-[36px] text-xs">
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white">
              <div className="text-[11px] font-bold uppercase tracking-[0.15em] opacity-90 mb-1">
                ⚽ FIFA World Cup 2026 · {match.label} · {match.group}
              </div>
              <h1 className="text-2xl sm:text-3xl font-bold leading-tight tracking-tight">
                {match.flagA} {match.teamA} <span className="opacity-70">vs.</span>{' '}
                {match.flagB} {match.teamB}
              </h1>
              <div className="text-sm opacity-90 mt-1">{match.date}</div>
            </div>
          </div>
